...
public class WorkShiftsBean implements Serializable {
    ...
    public void init() {
        try (Client client = ClientBuilder.newClient()) {
            this.work_shifts = client.target("http://localhost:8080/adminWebsite/api/work_shifts")
                    .request()
    ...
    public void addWorkShift() {
        try (Client client = ClientBuilder.newClient()) {
            try (Response response = client.target("http://localhost:8080/adminWebsite/api/work_shifts")
                    .request()
